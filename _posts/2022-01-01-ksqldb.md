...
# Conclusion
Even it emerged as a messaging platform, Apache Kafka is now a distributed event streaming technology used by myriad of organizations[^1] and teams to build high-performance, resilient, fault-tolerant data buses to have knowledge and insight via real-time analytics and data integration. Obviously, it is not possible to summarize Apache Kafka in a three parts article. The best way to comprehend such a state-of-art technology is to read the official documentations, books of experts and most importantly to apply the theory with setting up a basic cluster and coding against it. The references part of each article contains very valuable resources that pave the way of my Apache Kafka journey and writing this article series. 
...
![architecture](https://raw.githubusercontent.com/selcuksert/task-manager/main/docs/arch/task-manager.png)
